...
def load_data():
    ...
    # データを全行取得（位置指定で読み込むためKeyErrorを回避）
    rows = sheet.get_all_values()
    if len(rows) > 1:
        # ヘッダーを無視してデータのみ取得し、列名を強制固定
...
if not st.session_state.my_name:
    name_input = st.text_input("あなたの名前（4文字以内）", max_chars=4)
    if st.button("名前を確定"):
...
# --- 計算結果 ---
st.subheader("⚖️ 精算結果（誰がいくら誰に渡す？）")
if not df.empty:
    for session in df['会'].unique():
        st.write(f"### 📍 {session}")
        s_df = df[df['会'] == session]
        members = s_df['支払者'].unique()
        total = s_df['金額'].sum()
        avg = total / len(members)
        
        balance = {m: s_df[s_df['支払者'] == m]['金額'].sum() - avg for m in members}
